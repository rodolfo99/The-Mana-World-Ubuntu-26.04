...
test('real TCP login, registration, character creation, map, movement and play commands', { timeout: 15000 }, async t => {
  ...
  const map = await fakeServer(async socket => {
    ...
    const choices = packet(0x00b7, 14);
  ...
  assert.deepEqual((await ws.wait('dialog', m => m.choices?.length === 2)).choices, ['Sí', 'No']);
  session.command({ type: 'choice', id: 5200, index: 2 });
