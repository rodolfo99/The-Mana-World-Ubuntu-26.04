...
if ! node -e 'const [major,minor,patch]=process.versions.node.split(".").map(Number);process.exit(!((major===22&&(minor>22||minor===22&&patch>=3))||(major===24&&(minor>15||minor===15))||major>=26))'; then
